...
class Category extends ObjectModel
{
    ...
    public function recurseLiteCategTypeTree($max_depth = 3, $current_depth = 0, $id_lang = null, $excluded_ids_array = null)
    {
        $id_lang = is_null($id_lang) ? Context::getContext()->language->id : (int)$id_lang;

        $children = array();
        $subcats = $this->getSubCategoriesType($id_lang, true);
        if (($max_depth == 0 || $current_depth < $max_depth) && $subcats && count($subcats)) {
            foreach ($subcats as &$subcat) {
                if (!$subcat['id_category']) {
                    break;
                    ...
                    $categ = new Category($subcat['id_category'], $id_lang);
                    $children[] = $categ->recurseLiteCategTypeTree($max_depth, $current_depth + 1, $id_lang, $excluded_ids_array);
                }
            }
        }
    ...
    public function getSubCategories($id_lang, $active = true)
    {
		...
		'.$sql_groups_join.'
		WHERE cl.link_rewrite NOT LIKE "category%" AND `id_parent` = '.(int)$this->id.'
		'.($active ? 'AND `active` = 1' : '').'
		'.$sql_groups_where.'
		GROUP BY c.`id_category`
    ...
    public function getSubCategoriesType($id_lang, $active = true)
    {
        $sql_groups_where = '';
        $sql_groups_join = '';
        if (Group::isFeatureActive()) {
		...
		LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON (c.`id_category` = cl.`id_category` AND `id_lang` = '.(int)$id_lang.' '.Shop::addSqlRestrictionOnLang('cl').')
		'.$sql_groups_join.'
		WHERE cl.link_rewrite LIKE "category%" AND `id_parent` = '.(int)$this->id.'
		'.($active ? 'AND `active` = 1' : '').'
		'.$sql_groups_where.'
		GROUP BY c.`id_category`
		ORDER BY `level_depth` ASC, category_shop.`position` ASC');
...
* 	 * @param bool $method_to_create a category
    ...
    public function getParentsCategories($id_lang = null)
    {
        ...
        while (true) {
            ...
            $sql .= ' c.`id_category` = '.(int)$id_current;
            if (Shop::isFeatureActive() && Shop::getContext() == Shop::CONTEXT_SHOP) {
    ...
    */
    ...
    public function updatePosition($way, $position)
    {
        if (!$res = Db::getInstance()->executeS('
            ...
            WHERE cl.link_rewrite NOT LIKE "category%" AND cp.`id_parent` = '.(int)$this->id_parent.'
            ORDER BY category_shop.`position` ASC')
            ) {
            return false;
        }
